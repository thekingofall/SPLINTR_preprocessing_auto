---
title: "SPLINTR Library Barcode Preprocessing and Analysis (Multiple Samples)"
output:
  html_document:
    df_print: paged
    toc: true
...
rm(list=ls())
# Ensure necessary packages are installed before loading
required_packages <- c("ggplot2", "data.table", "dplyr", "assertthat",
                       "seqinr", "fastmatch", "venn", "VennDiagram",
                       "patchwork", "RColorBrewer", "futile.logger", "grid",
                       "scales", "UpSetR") # Added UpSetR

# Check and install missing packages (optional, uncomment if needed)
...
library(seqinr)
library(fastmatch)
# library(venn) # Not strictly needed anymore if UpSetR is used for >5 sets
library(VennDiagram)   # Used for saving Venn plots (2-5 sets)
library(patchwork)     # Used for combining plots
library(RColorBrewer)  # For plot colors
library(futile.logger) # To suppress VennDiagram log file
library(grid)          # For drawing Venn diagrams
library(scales)        # For plot axis labels (percent, scientific)
library(UpSetR)        # For visualizing overlaps of >5 sets

# Suppress log file generation by venn.diagram during knit/run
...
```{r setup-environment-paths}
# --- User Defined Parameters ---
# !!! MODIFY THESE PATHS AS NEEDED !!!
starcode_input_dir <- "/home/YangZongmian/SPLINTR/X101SC24127971-Z01-J027/Processed_SPLINTR_Output_Parallel/Step2_Starcode"  ## Input folder
analysis_output_dir <- "/home/YangZongmian/SPLINTR/X101SC24127971-Z01-J027/Processed_SPLINTR_Output_Parallel/Step3_SPLINTR_Analysis_Output" ## Output folder

# Minimum raw count threshold for filtering barcodes
raw_count_threshold <- 5 # Example: Set desired threshold (e.g., 5 or 20)
...
```{r setup-load-data, message=FALSE, warning=FALSE}
...
if (length(starcode_files) == 0) stop("FATAL ERROR: No 'Sample*_starcode.txt' files found in: ", starcode_input_dir)
...
if (length(starcode_data_list) == 0) stop("FATAL ERROR: No data could be loaded. Check input files.")
...
Determine max rank/count across samples for consistent axes in combined frequency plots.

```{r calculate-global-limits}
...
Apply `parse.starcode` to each sample, using global limits for consistent plot axes.
...
Generate plots showing all samples overlaid on the same axes.
...
Combine individual plots into summary figures using `patchwork`.
...
Analyze overlap between filtered barcode sets. Uses Venn diagrams for 2-5 samples and UpSet plots for >5 samples.

```{r barcode-overlap-upset}
  ...
  if (length(barcode_list_for_overlap) < 2) {
      ...
      if (num_sets > 5) {
          ...
          if (!is.null(upset_data)){
              ...
              # Define plotting parameters (can be adjusted)
              upset_params <- list(nsets = num_sets, nintersects = 40, order.by = "freq", decreasing = TRUE,
                                   ...
                                   text.scale = c(1.3, 1.3, 1, 1, 1.2, 1.1)) # Adjusted last element for intersection size numbers

              # Save as PDF
              tryCatch({ pdf(upset_plot_filename_pdf, onefile = FALSE, width = 10, height = 6); print(do.call(upset, c(list(data=upset_data), upset_params))); dev.off()
                         cat("Saved UpSet plot (PDF) to:", upset_plot_filename_pdf, "\n") }, error = function(e){ cat("Error saving UpSet plot (PDF):", e$message, "\n"); if(names(dev.cur()) != "null device") dev.off() })
              # Save as PNG
              tryCatch({ png(upset_plot_filename_png, width = 10, height = 6, units = "in", res=300, type="cairo"); print(do.call(upset, c(list(data=upset_data), upset_params))); dev.off()
